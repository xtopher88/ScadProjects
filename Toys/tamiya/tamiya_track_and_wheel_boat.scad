...
{
    wheel_base = 60.0;
    main_wheel_spacing = 120;
    idler_wheel_spacing = 65;
    idler_wheel_spacing1 = 105;
    idler_wheel_height = -18;
    idler_wheel_height1 = -2;
    block_height = 19.3;
    shaft_diameter = 3.0;
    ...
    module shaft_cutout()
    {
        ...
        rotate([90,0,0]) cylinder(wheel_base-15,d_inner/2,d_inner/2,center=true);
    ...
    difference()
    {
        union()
        {
            translate([0,(wheel_base/2+1),-6]) cube([130,6,32],center=true);
            translate([0,-(wheel_base/2+1),-6]) cube([130,6,32],center=true);
            
            //%translate([28,0,-19]) cube([6,16*4+4,6],center=true);
            translate([0,0,-18]) cube([90,16*4+4,8],center=true);
            
        rotate([0,39,0]) translate([-35,0,-36]) cube([36,16*4+4,12],center=true);
        rotate([0,-39,0]) translate([35,0,-36]) cube([36,16*4+4,12],center=true);
            
        translate([main_wheel_spacing/2,0,6]) cube([10,16*4+4,8],center=true);
        translate([-main_wheel_spacing/2,0,6]) cube([10,16*4+4,8],center=true);
        //rotate([0,-39,0]) translate([35,0,-59]) cube([main_wheel_spacing/2,16*4+10,block_height*2],center=true);
            ...
            difference()
            {
                union()
                {
                    ...
                    translate([0,0,9]) duplo_top(8,4);
                    // supports for the front blocks
                    translate([0,-24,3]) triangle_support(16*8+2,10);
                    translate([0,24,3]) rotate([-90,0,0]) triangle_support(16*8+2,10);
                }
                //remove the center of the front blocks
                translate([0,0,10]) cube([16*8+4,16*2+6,10],center=true);
            }
        }
        // front and rear angled bottom
        rotate([0,39,0]) translate([-35,0,-58]) cube([main_wheel_spacing/2,16*4+10,block_height*2],center=true);
        rotate([0,-39,0]) translate([35,0,-58]) cube([main_wheel_spacing/2,16*4+10,block_height*2],center=true);
        
        // drive shafts
        translate([main_wheel_spacing/2,0,6]) shaft_cutout();
        translate([idler_wheel_spacing1/2,0,idler_wheel_height1]) shaft_cutout();
